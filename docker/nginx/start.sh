...
if [ "$NGINX_ENABLE_SSL" = true ]; then
  ...
  if [ "$NGINX_FORCE_HTTPS" = true ]; then
    echo "Forcing HTTPS..."
    cat "$NGINX_INC_DIR/laravel_force_https.conf" > "$NGINX_INC_DIR/laravel_http_context.conf"
  else
    cat "$NGINX_INC_DIR/laravel_locations.conf" > "$NGINX_INC_DIR/laravel_http_context.conf"
  fi
else
  echo "Generating nginx configuration for http..."
  cat "$NGINX_INC_DIR/laravel_locations.conf" > "$NGINX_INC_DIR/laravel_http_context.conf"
  echo "" > "$NGINX_INC_DIR/laravel_https_server.conf"
fi
